...
export class BuildRunner {
  ...
  private executeBuild(): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      // For interpreted languages, no cwd means no build needed - that's OK
      if (!this.cwd || !this.cwd.trim()) {
        log.debug('No cwd specified, skipping build (interpreted language mode)');
        resolve(true);
        return;
      }

      log.debug({ command: this.command, cwd: this.cwd }, 'Starting build');
