...
import { ProcessTerminator, type TerminationOptions } from './process-terminator.js';
...
function createMockReadinessChecker(): ProcessReadinessChecker {
  return {
    waitUntilReady: vi.fn<ProcessReadinessChecker['waitUntilReady']>(),
    checkIntervalMs: 100,
    timeoutMs: 5000,
    settleDelayMs: 10
  };
...
describe('McpServerLifecycle', () => {
  ...
  let mockStopTerminator: ProcessTerminator;
  ...
  beforeEach(() => {
    ...
    // Create terminators with appropriate options
    mockStopTerminator = createMockTerminator({
      closeStdin: true,
      gracePeriodMs: 1000,
      forcePeriodMs: 2000,
      zombieTimeoutMs: 100,
    ...
    mockTerminatorTerminate(mockStopTerminator);
  ...
  describe('start', () => {
    ...
    it('should track unexpected process exits', async () => {
      ...
      // Assert - process should be cleared, allowing restart
  ...
  describe('stop', () => {
    it('should terminate process using stop terminator', async () => {
      // Arrange
      await lifecycle.start();
      ...
      // Assert
      expect(mockStopTerminator.terminate).toHaveBeenCalledWith(mockChildProcess);
    });

    it('should handle stop when not running', async () => {
      // Act & Assert - should not throw
      await expect(lifecycle.stop()).resolves.toBeUndefined();
      expect(mockStopTerminator.terminate).not.toHaveBeenCalled();
    });

    it('should clear process reference before terminating', async () => {
      // Arrange
      await lifecycle.start();
      const terminateSpy = vi.mocked(mockStopTerminator.terminate);

      // Act
      await lifecycle.stop();

      // Assert - process should be cleared before terminate is called
      expect(terminateSpy).toHaveBeenCalledOnce();
      // Attempting to stop again should be a no-op
      await lifecycle.stop();
      expect(terminateSpy).toHaveBeenCalledOnce();
    });
  });
