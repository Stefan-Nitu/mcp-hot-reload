...
describe('MessageRouter', () => {
  ...
  let messageQueue: MockedObject<MessageQueue>;
  let sessionTracker: MockedObject<SessionTracker>;
  ...
  beforeEach(() => {
    ...
    } as MockedObject<MessageQueue>;

    // Create mock with only the methods we use in tests
    sessionTracker = {
      processClientData: vi.fn((data: string) => data),
      processServerData: vi.fn((data: string) => data),
      getInitializeRequest: vi.fn(),
      isInitialized: vi.fn().mockReturnValue(false),
      reset: vi.fn()
    } as MockedObject<SessionTracker>;
  ...
  describe('message routing', () => {
    ...
    it('should track initialize requests via sessionTracker', () => {
      // Arrange
      router.connectServer(serverIn, serverOut);
      ...
      // Assert
      expect(sessionTracker.processClientData).toHaveBeenCalledWith(initRequest);
    });

    it('should track initialize responses via sessionTracker', () => {
      // Arrange
      router.connectServer(serverIn, serverOut);
      ...
      // Assert
      expect(sessionTracker.processServerData).toHaveBeenCalledWith(initResponse);
    });
    ...
    it('should handle multiple messages in one chunk', () => {
      ...
      expect(sessionTracker.processClientData).toHaveBeenCalledWith(messages);
    });
  });
  ...
  describe('error handling', () => {
    it('should queue messages when serverIn.write() throws exception', () => {
      ...
      const mockWrite = vi.fn<(chunk: any) => boolean>().mockImplementation(() => {
  ...
  describe('stream state validation', () => {
    ...
    it('should not write to clientOut when it is destroyed', () => {
      // Arrange
      const mockClientOut = new PassThrough();
      const mockWrite = vi.fn<(chunk: any) => boolean>().mockReturnValue(true);
      // TypeScript limitation: overloaded signatures require type assertion
      mockClientOut.write = mockWrite as typeof mockClientOut.write;
  ...
  describe('edge cases', () => {
    ...
    it('should handle data chunks with only whitespace', () => {
      ...
      // SessionTracker will be called but won't find any initialize messages
    });
    ...
    it('should handle binary data chunks', () => {
      ...
      // SessionTracker will be called with binary data converted to string
    });
  });
});