...
describe.sequential('MCPProxy E2E Tests', () => {
  ...
  describe('Real MCP Server Integration', () => {
    it('should work with a real MCP server implementation', async () => {
      ...
      const realServerPath = fixtures.TEST_SERVERS.REAL_MCP;
      ...
      proxy = new MCPProxy(
        {
          ...
          cwd: fixtures.PROJECT_ROOT,  // Run from project root where node_modules exists
    ...
    it('should handle real file changes and server restart', async () => {
      ...
      const fixtureServerPath = fixtures.TEST_SERVERS.VERSIONED_TEST;
...
`;
      ...
      await new Promise(resolve => setTimeout(resolve, 500));
      const streams = (proxy as any).serverLifecycle.getStreams();
      expect(streams).toBe(null);
    }, 10000);
  });
...
`;
      ...
      expect((proxy as any).serverLifecycle.getStreams()).not.toBe(null);
    }, 10000);
  });

  describe('Signal Handling', () => {
    it('should exit immediately on SIGINT and kill child process', async () => {
      // Arrange
      const client = new MCPTestClient();
      const proxyPath = path.resolve('dist/index.js');
      const serverPath = fixtures.TEST_SERVERS.SIGNAL_TEST;
      ...
      // Wait for proxy and server to fully start
      await new Promise(resolve => setTimeout(resolve, 500));
      ...
      // Assert
      expect(elapsed).toBeLessThan(250); // Should exit immediately
      expect(code).toBe(0); // Clean exit
      expect(client.isRunning()).toBe(false); // Process is dead
